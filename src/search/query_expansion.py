...
from search.question import INTENT_VOCAB, Question


@dataclass
class TermGroup:
    """一组语义等价的检索词（组内 OR）。"""
    role: str                      # entity / intent / year / topic
    terms: list[str] = field(default_factory=list)
    note: str = ""                 # 这一组怎么来的，写进 API 响应便于复核
...
class Expanded:
    ...
    def as_dict(self) -> dict:
        return {
            "groups": [{"role": g.role, "terms": g.terms, "note": g.note}
                       for g in self.groups if g],
            "all_terms": self.all_terms,
        }


def expand(q: Question) -> Expanded:
    """把分析好的问题扩展成检索词组。确定性——同样的问题永远同样的词表。"""
    exp = Expanded()

    # 组 1：实体（含别名）。这是权重最高的一组，也是召回的主力。
    # 分档见 _term_roles：规范名 / 已核实别名 / 语料派生短称，排序层按档给分。
    for e in q.entities:
        for role, term in _term_roles(e["entity"], e["matched"]):
            exp.groups.append(TermGroup(
                role=role, terms=[term],
                note=f"实体 {e['entity']}（{e['weight']}）：{e['why']}"))

    # 组 2：意图强词。古人的说法与今人不同，这一组负责把「死」扩成 卒/薨/崩/沒。
    # 这些词**基本只**用于这个意思，可以凭它们单独把段落召回来。
    for name in q.intents:
        spec = INTENT_VOCAB[name]
        terms = [t for t in spec["corpus"] if t not in ("于", "於")]
        if terms:
            exp.groups.append(TermGroup(
                role="intent", terms=terms,
                note=f"意图「{name}」的古代表达（语料实测词表，可独立召回）"))
        # 组 2b：意图弱词。也说这个意思，但更常用于别的意思，**只参与排序**，
        # 不单独召回 —— 否则「殺之」「終取之」这类段落会灌满召回池。
        weak = [t for t in spec.get("corpus_weak", []) if t not in terms]
        if weak:
            exp.groups.append(TermGroup(
                role="intent_weak", terms=weak,
                note=f"意图「{name}」的弱表达（只参与排序，不独立召回）"))

    # 组 3：年份。用户提了纪年就一定要用上。
    if q.years:
        exp.groups.append(TermGroup(
            role="year", terms=list(q.years), note="问句中的纪年表达"))

    # 组 4：没有实体时的兜底——把问句里的实词切出来当主题词。
    # 「城濮之战谁赢了」里 城濮 不是人物实体，但它是检索的关键。
    if not q.entities:
        topics = _topic_terms(q.text)
        if topics:
            exp.groups.append(TermGroup(
                role="topic", terms=topics,
                note="问句切分得到的主题词（无人物实体时使用）"))

    exp.asks_duration = q.asks_duration
    return exp


def _aliases_of(entity: str) -> list[str]:
    return entities.aliases_of(entity)


def _shared_aliases() -> set[str]:
    ...
    第 4 位（实测 rel=29.0，仅次于是齊桓公本人的几段），而正文里根本没有「齊」。
    """
    return {al for al, dist in entities.short_forms().items() if len(dist) > 1}


def _term_roles(entity_name: str, matched: str) -> list:
    ...
    """
    curated = entities.curated().get(entity_name, {})
    curated_aliases = set(curated.get("aliases", []))
    shared = _shared_aliases()

    roles: list[tuple[str, str]] = [("entity", entity_name)]
    for al in _aliases_of(entity_name):
        if al in shared:
            # 两家共用的别名（夷吾）：语料里是它，但**不敢说是谁**，按候选档。
            roles.append(("weak", al))
        elif al in curated_aliases:
            ...
            if entities.occurs_in_corpus(al):
                roles.append(("alias", al))
        elif al in entities.corpus_names():
            roles.append(("weak", al))       # 只有语料频次的短称（文公/穆公）
        # 语料里查不到的别名：不参与检索，也不报给用户当检索词
    if matched and matched not in [t for _, t in roles]:
        roles.append(("alias" if (matched in curated_aliases
                                  and matched not in shared) else "weak", matched))
    return roles


# 问句里的功能词/疑问词，切主题词时去掉
_STOPWORDS = set("的了是在有和与與及之其所以为何何誰谁什么什麼怎么怎麼如何"
                 "吗嗎呢吧啊请问請問是什么是不是多少哪哪个個这這那哪些"
                 "年月份日时候時候关系關係原因经过經過事件事情"
                 "贏赢输輸人")
# 问句里成段的功能短语，先整体摘掉再切
_STOP_PHRASES = ("是什么", "什么关系", "怎么回事", "什么时候", "为什么",
...
def _topic_terms(text: str) -> list[str]:
    ...
    for p in _STOP_PHRASES:
        s = s.replace(p, "")
    out: list[str] = []
    for run in _HAN_RUN.findall(s):
        # 只削句尾的泛称/疑问字（「董卓人」→「董卓」），不削句首 ——
        # 句首的「人」可能是主体的一部分（人傑、人臣），削错了就是替用户改问题。
        run = run.rstrip("".join(_TAIL_NOISE))
        i = 0
        while i < len(run):
            for size in (4, 3, 2):
                seg = run[i:i + size]
                if len(seg) == size and not any(c in _STOPWORDS for c in seg):
                    if seg not in out:
                        out.append(seg)
                    i += size
                    break
...
_HAN_RUN = re.compile("[一-鿿]+")
# 年数表达：十二年 / 十九年 / 三年 / 三百餘年。「多少年」里没有年数，所以必须
# 先有个汉字数词再跟「年」，不能只匹「年」。
_RE_YEAR_COUNT = re.compile(
    "[一二三四五六七八九十百千萬万]{1,6}餘?年|[兩两三四五六七八九十]年")
# 句尾的泛称/疑问字，切主题词时削掉
_TAIL_NOISE = set("人誰谁贏赢輸输呢吗嗎吧啊了")


def _split_han(text: str) -> list[str]:
    """连续汉字串按 2–4 字滑窗切，保留较长的优先。"""
    out: list[str] = []
